 ...
 */
export const validateJWT = pinataJWT => {
    if (!pinataJWT || pinataJWT === '')
        throw new Error('No pinataJWT provided! Please provide your pinata api key as an argument when you start this script')
}
 ...
 * @param {*} hostNodes
 */
export const validateHostNodes = hostNodes => {
...
export const validateMetadata = metadata => {
    ...
    if (metadata.keyvalues) {
        ...
        Object.entries(metadata.keyvalues).forEach(keyValue => {
            if (i > 9)
 ...
 * @param {*} pinPolicy
 */
export const validatePinPolicyStructure = pinPolicy => {
 ...
 * @param {*} options
 */
export const validatePinataOptions = options => {
